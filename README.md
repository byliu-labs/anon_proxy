...
| `--log-json` | off | Emit supported operational events, including metrics, as JSON lines instead of human text |
...
Use `--log-json` with `--metrics` when logs are consumed by another process:

```bash
uv run python -m anon_proxy.server --metrics --log-json
```

JSON event lines exclude raw request, response, and detected entity text. Human
debug output can still show masked/unmasked diffs when `--debug` is enabled.
