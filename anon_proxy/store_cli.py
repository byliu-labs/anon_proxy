...
from anon_proxy.mapping import _parse_token, _placeholder_label, atomic_write_json
...
def filter_entries(
    ...
    normalized_label = _placeholder_label(label) if label is not None else None
...
def purge_tokens(data: dict, tokens: list[str]) -> tuple[dict, list[str]]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "show":
        ...
        if value is None:
            print(f"{args.token} not found", file=sys.stderr)
            return 1
        print(value)
        return 0

    if args.command == "purge":
        new_data, removed = purge_tokens(data, args.tokens)
        if not removed:
            return 0
    ...
    if args.command == "prune":
        ...
        new_data, removed = purge_tokens(data, tokens)
        for token in removed:
